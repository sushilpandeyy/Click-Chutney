  ...
  async signInWithGitHub() {
    try {
      ...
      if (result.error) {
        return {
          success: false,
          error: getSpicyErrorMessage(result.error.message ?? "GitHub sign-in failed"),
        }
      }
      ...
      return {
        success: false,
        error: "GitHub connection failed! Even the Octocat is confused 🐙",
      }
    }
  },
...
function getSpicyErrorMessage(error: string): string {
  const errorMessages: Record<string, string> = {
    ...
    "GitHub sign-in failed": "GitHub didn't want to share their code! Try again 🐙",
    "OAuth error": "OAuth got confused! Like mixing up salt and sugar 🧂",
  }
